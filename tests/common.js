...
const PG_CONTAINER = process.env.PG_CONTAINER_NAME;
...
export function resetdb () {
  ...
  if (have_psql) {
    ...
    pg = spawnSync('docker', ['exec', PG_CONTAINER, 'psql', '-U', SUPER_USER, DB_NAME, '-f', 'docker-entrypoint-initdb.d/sample_data/reset.sql'])
  }
  if (pg.status !== 0) {
